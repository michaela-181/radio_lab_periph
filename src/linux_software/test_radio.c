...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    for (int i=0;i<2048;i++)
        stop_time = *(periph_base+RADIO_TUNER_TIMER_REG_OFFSET); // read the clock counter
    ...
    unsigned int bytes_transferred = 2048 << 2; // change obviously
    float time_spent = (stop_time-start_time)/125.0e6; // change obviously
	float throughput= (bytes_transferred)/1.0e6/time_spent; 
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Michaela Mitchell - Custom Peripheral Demonstration\n\r");
